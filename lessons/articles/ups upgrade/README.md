### [Модернизация ИБП APC 650, добавление в Home Assistant, получение фактической емкости аккумулятора](https://youtu.be/SAfi_6aKyfE)
          ...
          - name: lr_ups_1_load
            state: > 
                {{
                  ...
                  states('sensor.0xa4c13850763b90bf_power')|float(0)
                }}  
...
### Сохранение энергии отданной ИБП
    ...
    # Сенсор электрики на выходе из UPS
        ...
        - service: mqtt.publish
          data_template:
            topic: "states/lr_ups_1_power"
            payload_template: "{{(states('sensor.lr_ups_1_power') | float + (states('sensor.0xa4c13850763b90bf_power') | float /120))|round(5) }}"
            retain: true 
                  ...
                  - service: telegram_bot.send_message
                    data_template:
                      ...
                      message: | 
                           ...
                           {{"\U0001F50B"}} Отдано {{ states('sensor.lr_ups_1_power') }} Ватт часов энергии
              - conditions:
                  ...
                  - service: telegram_bot.send_message
                    data_template:
                      ...
                      message: | 
                           ...
                           {{"\U0000231B"}} {{ 'UPS 1 продержал - {:02d}:{:02d}:{:02d}'.format ( s % 86400 // 3600, s % 3600 // 60, s % 60) }}
                           {{"\U0001F50B"}} Отдано {{ states('sensor.lr_ups_1_power') }} Ватт часов энергии, {{ ((states('sensor.lr_ups_1_power') | float / 108)*100) | round(3)}} % от номинальной емкости
